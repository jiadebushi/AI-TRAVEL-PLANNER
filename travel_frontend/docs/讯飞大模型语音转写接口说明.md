...
def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    # 这里实现token验证逻辑
    # 如果验证失败，抛出 HTTPException(status_code=401)
    return token

@router.get("/voice/xunfei-llm/ws-url")
async def get_xunfei_llm_ws_url(token: str = Depends(verify_token)):
    """获取讯飞大模型 WebSocket URL"""
    try:
        # 检查配置
        if not all([XUNFEI_LLM_APP_ID, XUNFEI_LLM_ACCESS_KEY_ID, XUNFEI_LLM_ACCESS_KEY_SECRET]):
            raise HTTPException(status_code=500, detail="讯飞配置缺失")
        ...
        # 准备参数
        params = {
            'accessKeyId': XUNFEI_LLM_ACCESS_KEY_ID,
            'appId': XUNFEI_LLM_APP_ID,
            'audio_encode': 'pcm_s16le',
            'lang': 'autodialect',
            'samplerate': '16000',
            'utc': utc_str,
        ...
        # 生成签名
        hmac_sha1 = hmac.new(
            XUNFEI_LLM_ACCESS_KEY_SECRET.encode('utf-8'),
            base_string.encode('utf-8'),
            hashlib.sha1
        ).digest()
        signature = base64.b64encode(hmac_sha1).decode('utf-8')
        ...
        raise HTTPException(status_code=500, detail=f"生成WebSocket URL失败: {str(e)}")
```
