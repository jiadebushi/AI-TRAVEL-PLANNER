...
**重要约束:**
7. **预算控制:** 根据用户的预算，你给出的总预算 (`estimated_total_cny`) 必须控制在用户预算的 **60% - 85%** 之间。即：
   - 最低预算 = 用户预算 × 0.60
   - 最高预算 = 用户预算 × 0.85
   - 你估算的总预算必须落在此区间内

请估算总预算和各项开支的细分（住宿、餐饮、交通、门票、购物、其他），注意只有这6项，不要添加其他项，并以JSON格式返回。
...
{{
  ...
  "categories": [
    ...
    {{"name": "购物", "estimated_cny": 金额}},
    {{"name": "其他", "estimated_cny": 金额}}
  ]
}}
"""
...
请提取以下信息：
- category: 开销类别（餐饮、交通、住宿、门票、购物、 其他），注意只有这6类，尽量分类，无法分类的归于其他。
- amount: 金额（浮点数）
- currency: 货币单位（默认CNY）
- description: 描述（字符串）
